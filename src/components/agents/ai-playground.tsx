    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          toast.error('Aún no hay un agente configurado — primero completa la Configuración.');
        } else {
        ...
        {turns.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
            ...
            <p className="mt-1 text-xs">
              Usa tu base de conocimientos y se comporta igual que el bot de
              respuesta automática, incluida la derivación a un humano.
            </p>
            {onGoToSetup && (
              <Button
                variant="link"
              ...
              >
                ¿Aún no está configurado? Ve a Configuración <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            )}
          </div>
        )}
