...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error('Vuelve a ingresar el token de acceso para guardar los cambios');
        setSaving(false);
        return;
      }
      ...
      if (data.registered === false && data.registration_error) {
        ...
        toast.success(
          'Credenciales guardadas y verificadas. Se omitió el registro entrante (sin PIN) — consulta el estado de registro abajo.',
          { duration: 10000 },
        );
        setPin('');
      } else {
        toast.success(
          data.phone_info?.verified_name
            ? `En vivo — ${data.phone_info.verified_name} ya puede recibir eventos.`
  ...
  async function handleTestConnection() {
    try {
      ...
      if (payload.connected) {
        ...
        setResetReason(payload.needs_reset ? 'token_corrupted' : payload.reason === 'meta_api_error' ? 'meta_api_error' : null);
        setStatusMessage(payload.message || '');
        toast.error(payload.message || 'Error de conexión con la API');
      }
    } catch (err) {
      console.error('Test connection error:', err);
      setConnectionStatus('disconnected');
      toast.error('Falló la prueba de conexión. Revisa tu red e intenta de nuevo.');
    } finally {
      setTesting(false);
    }
  }
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      if (data.live) {
        toast.success('El número está completamente conectado — Meta está entregando eventos.');
      } else {
        toast.error(
          'El número no está completamente registrado. Consulta las verificaciones abajo para saber qué paso falló.',
          { duration: 8000 },
        );
      }
      if (accountId) await fetchConfig(accountId);
  ...
  async function handleReset() {
    if (!confirm('Esto eliminará la configuración actual de WhatsApp para que puedas volver a ingresarla. ¿Continuar?')) {
      return;
    }
    ...
    try {
      ...
      toast.success('Configuración eliminada. Ahora puedes volver a ingresar tus credenciales.');
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
      setAccessToken('');
