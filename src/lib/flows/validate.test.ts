...
describe("validateFlowForActivation — nodes", () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "b" &&
          i.field === "buttons" &&
          i.message.includes("al menos un"),
      ),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "b" &&
          i.field === "buttons" &&
          i.message.includes("como máximo 3"),
      ),
    ).toBe(true);
  });
  ...
  it("flags duplicate button reply_ids", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("ID de respuesta de botón duplicado")),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "l" &&
          i.field === "sections" &&
          i.message.includes("como máximo 10"),
      ),
    ).toBe(true);
  });
  ...
  it("flags list row title over 24 chars", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("supera los 24 caracteres")),
    ).toBe(true);
  });
