...
export function validateInteractivePayload(
  ...
  if (typeof p.body !== 'string' || p.body.trim() === '') {
    return fail('El texto del mensaje interactivo es obligatorio.')
  }
  ...
  if (p.kind === 'buttons') {
    ...
    if (buttons.length > INTERACTIVE_LIMITS.maxButtons) {
      return fail(
        `Un mensaje de botones de respuesta permite como máximo ${INTERACTIVE_LIMITS.maxButtons} botones.`,
      )
    }
    ...
    for (const b of buttons) {
      ...
      if (seen.has(b.id)) {
        return fail(`ID de botón duplicado: "${b.id}".`)
      }
      seen.add(b.id)
  ...
  if (p.kind === 'list') {
    ...
    if (
      ...
      return fail('La lista necesita una etiqueta de botón.')
    }
    ...
    for (const section of list.sections) {
      ...
      for (const row of section.rows) {
        ...
        if (seen.has(row.id)) {
          return fail(`ID de fila de lista duplicado: "${row.id}".`)
        }
        seen.add(row.id)
  ...
  return fail('El mensaje interactivo debe ser botones de respuesta o una lista.')
}
